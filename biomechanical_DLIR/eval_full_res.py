...
def save_sample_full_size(params, data, ddf, name):
    ...
    if ddf.shape[2::] != moving.shape:
        ...
        transform_resize = MTransforms.Resize((z,h,w), mode="trilinear", anti_aliasing=True, align_corners=True)
...
def main(args):
    ...
    # Get config path : 
    model_config_path = os.path.join(*args.checkpoint.split(os.sep)[:-2],"model_config.json")
    ...
    if args.out_folder:
        ...
        params["out_folder"] = os.path.join(*args.checkpoint.split(os.sep)[:-2]).replace(params["dataset"],params["dataset"]+"/eval")
        params["out_folder"] += "_"+args.checkpoint.split(os.sep)[-1].replace(".pth","")
    if args.mode!="val":
    ...
    with torch.no_grad():
        ...
        for i, (data, data_full_size) in enumerate(zip(valDL_normal,valDL_full_size)):
            ...
            metric_values = compute_metrics(data,torch.zeros_like(ddf),metric_funcs_no_registration,metric_values,prefix="before_")
            metric_values["time"].append(end_time-start_time)
            for key in metric_values:
                sample_metrics[key] = metric_values[key][-1]#np.mean(metric_values[key])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--checkpoint", default=None, help="Path to pretrained models. During training, continue from this checkpoint. During evaluation, evaluate this checkpoint.")
    ...
    parser.add_argument('--out_folder','-o', default=None, help='Path to save outputs or None')
