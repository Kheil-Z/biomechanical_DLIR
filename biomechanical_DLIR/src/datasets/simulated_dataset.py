        ...
        for i in range(self.shape[0]):
            for j in range(self.shape[1]):
                for k in range(center_z-10,center_z+10):
                    if (i<center_x+4 + (j-center_y)*np.sin(rotation)) and (i>center_x-4+ (j-center_y)*np.sin(rotation)):
                        if (j<center_y+15-(i-center_x)*np.sin(rotation)) and (j>center_y-15-(i-center_x)*np.sin(rotation)):
                            im[i,j,k]=1.
                            mask[i,j,k]=1
        

        #3) smooth the image
        im[1:-1,1:-1,1:-1]=(im[1:-1,1:-1,1:-1]
                            +im[1:-1,1:-1,0:-2]
                            +im[1:-1,1:-1,2:]
                            +im[1:-1,0:-2,1:-1]
                            +im[1:-1,2:,1:-1]
