...
Dataset called "Shearing" in paper
...
class SimulatedShearDataset(BaseDataset):
    ...
    def _generate_rotating_image(self):
        ...
        return im,mask,proj_vector
    
    def make_data_dict(self):
        data_dict = []
        for i in range(self.params["data_len"][self.mode]):
            fixed_im,_,_ = self._generate_rotating_image()
            moving_im, loss_mask, proj_vector = self._generate_rotating_image()
            data = {
                    "fixed": fixed_im,
                    "moving": moving_im,
                    "loss_mask": loss_mask,
