...
shear_stress_with_radius = [
    ...
    (10, "lung_upper_lobe_left", 43, "vertebrae_T1", 4),  # Left lung & upper thoracic spine
    (12, "lung_upper_lobe_right", 43, "vertebrae_T1", 4),  # Right lung & upper thoracic spine
    (11, "lung_lower_lobe_left", 34, "vertebrae_T9", 4),  # Left lung & lower thoracic spine
    (14, "lung_lower_lobe_right", 34, "vertebrae_T9", 4),  # Right lung & lower thoracic spine
    ...
    (1, "spleen", 10, "lung_lower_lobe_left", 5),  # Spleen & lung via diaphragm movement
    ...
    (5, "liver", 10, "lung_upper_lobe_left", 6), # <- (5, "liver", 11, "lung_lower_lobe_left", 6),  
    (10, "lung_upper_lobe_left", 34, "vertebrae_T9", 4), # <- (11, "lung_lower_lobe_left", 34, "vertebrae_T9", 4)
    (13, "lung_middle_lobe_right", 34, "vertebrae_T9", 4), # <- (14, "lung_lower_lobe_right", 34, "vertebrae_T9", 4)
    (5, "liver", 13, "lung_middle_lobe_right", 5), # <- (5, "liver", 14, "lung_lower_lobe_right", 5)
    (1, "spleen", 11, "lung_lower_lobe_left", 5) # <- (1, "spleen", 10, "lung_lower_lobe_left", 5)
]
...
def main(args):
    ...
    for path in paths_all:
        ...
        files_all.append(
            {
                "total": path.replace("imagesTr","segmentationsTr/total/").replace("imagesTs","segmentationsTs/total/"),
            }
        )
    ...
    for data_dict in files_all:
        ...
        mask_strain_directions = torch.zeros(3,total.shape[0],total.shape[1],total.shape[2])
        ...
        for label1,name1,label2,name2, radius in shear_stress_with_radius:
            ...
            if mask.int().max() == 0:
                ...
                print(f" ---- found {mask.int().sum().item()} between {name1} and {name2}")
                loss_mask[mask] = 2
        ...
        if not os.path.exists(save_folder):
            os.makedirs(save_folder)
        nib.save(
            nib.Nifti1Image(mask_strain_directions.squeeze().permute(1,2,3,0).cpu().numpy(), affine),
            save_name)


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument('--path', type=os.path.abspath, default="Datasets/AbdomenCTCT_reoriented/imagesTr", help='Path toLearn2Reg train data locations',)
