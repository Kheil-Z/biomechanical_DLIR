...
def reorient_to_rpi(nifti_img):
    ...
    current_ornt = nib.orientations.axcodes2ornt(nib.orientations.aff2axcodes(nifti_img.affine))
    ...
    for file in os.listdir(input_dir):
        if file.endswith(".nii") or file.endswith(".nii.gz"):
            ...
            if current_orientation == ('R', 'A', 'S'):
                ...
                print(f"------- Skipping {file}, not in LPI orientation. ({current_orientation})")
...
def main(args):
    ...
    convert_nifti_directory(input_dir.replace("imagesTr","imagesTs"), output_dir.replace("imagesTr","imagesTs"))
    convert_nifti_directory(input_dir.replace("imagesTr","labelsTr"), output_dir.replace("imagesTr","labelsTr"))

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument('--path', type=os.path.abspath, default="Datasets/AbdomenCTCT/imagesTr", help='Path toLearn2Reg train data locations',)
